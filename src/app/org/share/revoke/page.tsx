...
export default function RevokeSharePage() {
  ...
  const revoke = async () => {
    try {
      ...
      const res = await revokeShare(shareId.trim());
      ...
      <main className="relative min-h-screen pt-28 pb-24">
        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="mt-3 text-white/70">
            Revoke an on-chain share by shareId.
          </p>

          <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-6 grid gap-4">
            <div>
              <label className="block text-sm text-white/70">Share ID</label>
              <input
                value={shareId}
